...
function findBestHospital(patientCoords, hospitals, ambulanceCoords) {
  ...
  for (const hospital of hospitals) {
    ...
    const availabilityScore =
      100 - (hospital.available / hospital.capacity) * 100;
    const levelBonus = hospital.level.includes("Level I")
      ? 0
      : hospital.level.includes("Level II")
        ? 5
        : 10;
...
function generateRoute(startCoord, endCoord, waypoints = 8) {
  ...
  for (let i = 1; i < waypoints; i++) {
    ...
    const lng =
      startCoord[0] +
      (endCoord[0] - startCoord[0]) * t +
      (Math.random() - 0.5) * jitter;
    const lat =
      startCoord[1] +
      (endCoord[1] - startCoord[1]) * t +
      (Math.random() - 0.5) * jitter;
    route.push([lng, lat]);
  }
  ...
  for (const base of endpoints) {
    try {
      ...
      if (error.name === "AbortError") {
        console.log("OSRM request timed out, using fallback");
      }
      // Try next endpoint or return null
      continue;
    }
  }
...
async function snapToRoad(coord) {
  const [lng, lat] = coord;
  const data = await osrmRequest(`/nearest/v1/driving/${lng},${lat}?number=1`);
...
async function generateFullRoute(
  ambulanceCoords,
  patientCoords,
  hospitalCoords,
) {
  ...
  const toPatientRoad = await generateRoadRoute(
    snappedAmbulance,
    snappedPatient,
  );
  const toHospitalRoad = await generateRoadRoute(
    snappedPatient,
    snappedHospital,
  );

  const toPatient =
    toPatientRoad || generateRoute(snappedAmbulance, snappedPatient, 8);
  const toHospital =
    toHospitalRoad || generateRoute(snappedPatient, snappedHospital, 12);
