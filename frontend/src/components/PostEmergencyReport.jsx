...
function ReportCard({ report, index }) {
  ...
  const date = new Date(report.completedAt).toLocaleTimeString("en-US", {
    hour12: false,
  });
  const mins = Math.round(report.responseTimeSec / 60);
  const dist = report.totalDistance
    ? Number(report.totalDistance).toFixed(2)
    : "N/A";
  ...
  const tc =
    {
      ...
      heavy: "#f97316",
      severe: "#ef4444",
    }[report.trafficCondition] || "#f59e0b";
          ...
          >
            ...
            >
              {[
                ...
                {
                  label: "Traffic Delay",
                  value: report.trafficDelay || "< 1 min",
                  color: tc,
                },
                {
                  label: "Outcome",
                  value: report.outcome || "Delivered",
                  color: "#34d399",
                },
                {
                  label: "Ambulance",
                  value: report.ambulance?.id || "N/A",
                  color: "#f9a8d4",
                },
                {
                  label: "Traffic",
                  value: (report.trafficCondition || "moderate").toUpperCase(),
                  color: tc,
                },
              ].map((item) => (
                <div
                ...
                >
                  ...
                  <div
                    style={{ color: item.color, fontWeight: 700, fontSize: 13 }}
                  >
                    {item.value}
                  </div>
                </div>
